...
`clambot onboard` scans your environment variables, probes local Ollama, and generates `~/.clambot/config.json` with everything it finds. No manual editing needed.
...
ClamBot supports multiple LLM backends through a registry-driven provider layer. Set an API key via environment and run `clambot onboard` — the provider is auto-detected.
...
| `openai_codex` | ⚡ LLM (Codex, OAuth) | `clambot provider login openai-codex` |
| `custom` | 🔌 Any OpenAI-compatible endpoint | Config only — see below |

```bash
# Example: set up with OpenRouter
export OPENROUTER_API_KEY="sk-or-v1-xxx"
clambot onboard    # auto-detects provider + model
clambot status     # verify provider is ready ✅
clambot agent      # start chatting 💬
```
...
Config file: `~/.clambot/config.json` (auto-generated by `clambot onboard`)
...
| Command | Description |
|---------|-------------|
| `clambot onboard` | 🎬 Initialize config & workspace (auto-detects providers) |
| `clambot agent -m "..."` | 💬 Run a single agent turn |
| `clambot agent` | 🔄 Interactive chat mode (REPL) |
| `clambot gateway` | 🚀 Start the gateway (Telegram + cron + heartbeat) |
| `clambot status` | ✅ Show provider readiness |
...
| `clambot cron add --name "daily" --message "Hello" --cron "0 9 * * *"` | ➕ Add a cron job |
| `clambot cron remove <job_id>` | ❌ Remove a cron job |
